...
with Timer("Initializing data"):
    ...
    y = spline(x)
    xx = np.repeat(x, 10)
    yy = spline(xx) + (xx**2 + 0.1) * np.random.randn(*xx.shape)
...
with Timer("Running linear spline quantile regression with sklearn"):
    skllsq = {}
    for q in quantiles:
        skllsq[q] = LinearSpline(np.linspace(0, 1, 8), None)
...
with Timer("Running linear spline quantile regression with statsmodels"):
    ...
    for q in quantiles:
        smlsq[q] = LinearSpline.from_data(
            xx, yy, knots=knots, q=q, method="QuantileRegression"
        )
        print(".", end="")
...
with Timer("Running natural cubic spline quantile regression with statsmodels"):
    ...
    for q in quantiles:
        smcsq[q] = NaturalCubicSpline.from_data(
            xx, yy, knots=knots, q=q, method="QuantileRegression", max_iter=int(1e4)
        )
        print(".", end="")
...
with Timer("Plotting"):
    ...
    ax.plot(x, spline(x), label="Signal and its quantiles", color="k")
    print(".", end="")
    ax.scatter(xx, yy, alpha=0.1, label="Observed signal + noise")
    print(".", end="")
    for q in quantiles:
        ax.plot(x, spline(x) + (x**2 + 0.1) * sps.norm.ppf(q), label=None, color="k")
        print(".", end="")
